...
/// Extract value for less common types - separate function to keep hot path optimized
#[inline(never)]
fn extract_value_slow_path(row: &Row, i: usize, column_type: tiberius::ColumnType) -> PyResult<PyValue> {
    ...
    match column_type {
        ...
        tiberius::ColumnType::Numericn => {
            ...
            match row.try_get::<Numeric, usize>(i) {
                Ok(Some(numeric)) => {
                    ...
                    Ok(PyValue::new_float(float_val))
                },
                Ok(None) => Ok(PyValue::new_null()),
                Err(_) => {
                    // Fallback to string conversion
                    match row.try_get::<&str, usize>(i) {
                        Ok(Some(val)) => {
                            if let Ok(parsed) = val.parse::<f64>() {
                                Ok(PyValue::new_float(parsed))
                            } else {
                                Ok(PyValue::new_string(val.into()))
                            }
                        },
                        _ => Ok(PyValue::new_null()),
                    }
        ...
        tiberius::ColumnType::Daten => {
            match row.try_get::<NaiveDate, usize>(i) {
                Ok(Some(val)) => Ok(PyValue::new_datetime(val.format("%Y-%m-%d").to_string())),
                Ok(None) => Ok(PyValue::new_null()),
                Err(_) => {
                    // Fallback to string
                    match row.try_get::<&str, usize>(i) {
        ...
        tiberius::ColumnType::Timen => {
            match row.try_get::<NaiveTime, usize>(i) {
                Ok(Some(val)) => Ok(PyValue::new_datetime(val.format("%H:%M:%S%.f").to_string())),
                Ok(None) => Ok(PyValue::new_null()),
                Err(_) => {
                    // Fallback to string
                    match row.try_get::<&str, usize>(i) {
        ...
        tiberius::ColumnType::Datetime => {
            match row.try_get::<NaiveDateTime, usize>(i) {
                Ok(Some(val)) => Ok(PyValue::new_datetime(val.format("%Y-%m-%d %H:%M:%S%.f").to_string())),
                Ok(None) => Ok(PyValue::new_null()),
                Err(_) => {
                    // Fallback to string
                    match row.try_get::<&str, usize>(i) {
        ...
        tiberius::ColumnType::DatetimeOffsetn => {
            match row.try_get::<DateTime<Utc>, usize>(i) {
                Ok(Some(val)) => Ok(PyValue::new_datetime(val.to_rfc3339())),
                Ok(None) => Ok(PyValue::new_null()),
                Err(_) => {
                    // Fallback to string
                    match row.try_get::<&str, usize>(i) {
        ...
        tiberius::ColumnType::Image => {
            match row.try_get::<&[u8], usize>(i) {
                Ok(Some(val)) => Ok(PyValue::new_bytes(val.to_vec())),
                _ => Ok(PyValue::new_null()),
            }
        }
        ...
        tiberius::ColumnType::Guid => {
            match row.try_get::<Uuid, usize>(i) {
                Ok(Some(val)) => Ok(PyValue::new_string(val.to_string())),
                Ok(None) => Ok(PyValue::new_null()),
                Err(_) => {
                    // Fallback to string
                    match row.try_get::<&str, usize>(i) {
        ...
        tiberius::ColumnType::Xml => {
            ...
            if let Ok(Some(xml_data)) = row.try_get::<&XmlData, usize>(i) {
                Ok(PyValue::new_string(xml_data.to_string()))
            }
            // Fallback to string
...
impl PyValue {
    ...
    /// Convert PyValue to a Tiberius ToSql parameter - optimized for performance
    #[inline]
    pub fn to_sql(&self) -> Result<Box<dyn tiberius::ToSql>, String> {
        match &self.inner {
            ...
            PyValueInner::DateTime(s) => {
                // Try to parse as various datetime formats - fast path first
