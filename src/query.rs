...
impl PyQuery {
    ...
    pub fn execute(&self, connection: &PyConnection) -> PyResult<Vec<PyRow>> {
        connection.execute(self.sql.clone())
    }
    
    /// Execute the query without returning results
    pub fn execute_non_query(&self, connection: &PyConnection) -> PyResult<u64> {
        connection.execute_non_query(self.sql.clone())
    }
